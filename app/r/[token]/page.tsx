...
export default async function ClientReferralPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const withCats = (providers || []).map((p) => ({
    ...p,
    cats: (cats || []).filter((c) => c.provider_id === p.id),
  }))
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '16px 40px 64px' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#2c4d52', marginBottom: 8 }}>
            {referral.client_name ? `${referral.client_name}, here ${withCats.length > 1 ? 'are' : 'is'} your referral${withCats.length > 1 ? 's' : ''}` : 'Your referral'}
          </h1>
          <p style={{ fontSize: 15, color: '#666', lineHeight: 1.6 }}>
            {withCats.length > 1
              ? `You've been referred to ${withCats.length} trusted providers. Review them below and reach out to whichever feels like the best fit for you.`
